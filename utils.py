...
def normalize_edge(edge_index, num_nodes):
    ...
    return torch.sparse.FloatTensor(edge_index, edge_weight, num_nodes)
...
def data_pad(data, extra_id, dataset, mapping):
    ...
    for extra_data in extra_dataloader:
        ...
        row, col, _ = extra_data.adj_t.coo()
        row, col = row + data.x.size(0), col + data.x.size(0)
        tmp_adj_t = SparseTensor(row = row, col = col, value = None)

        data.adj_t = data.adj_t + tmp_adj_t
        ...
        data.ptr = torch.cat(
            [data.ptr[:-1], extra_data.ptr + data.ptr[-1]])
        # print(data.ptr.shape, extra_data.ptr.shape)
