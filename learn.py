...
def train(epoch, encoder, classifier, data_loader, dataset, optimizer_e, optimizer_c, args):
    ...
    for data in data_loader:
        if(args.setting in ['no', 'upsampling'] or args.min_num_graph == 0):
            ...
            knn_deg = degree(col, data.id.size(0), dtype=data.x.dtype)
            ...
            for i in range(args.prop_epochs):
                H_knn = torch.sparse.mm(adj_t, H_knn)
            ...
            for i in range(args.aug_num):
                if(args.aug == 'RE'):
                    ...
                    edge_index_aug = remove_edge(edge_index, args.aug_ratio)
                    adj_t = SparseTensor(row=edge_index_aug[0], col=edge_index_aug[1], value=None, sparse_sizes=(
                        data.x.size(0), data.x.size(0)))
                    H_augs.append(encoder(data.x, adj_t, data.ptr))
                    logit_augs.append(classifier(H_augs[-1]))
                elif(args.aug == 'DN'):
                    x_aug = drop_node(data.x, args.aug_ratio)
                    H_augs.append(encoder(x_aug, data.adj_t, data.ptr))
                    logit_augs.append(classifier(H_augs[-1]))
            ...
            for k in range(args.aug_num):
                H_aug_knn = H_augs[k]
                for i in range(args.prop_epochs):
                    H_aug_knn = torch.sparse.mm(adj_t, H_aug_knn)
